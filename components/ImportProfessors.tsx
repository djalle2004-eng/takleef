...
interface ImportProfessorsProps {
  onClose: () => void;
  onSuccess: () => void;
}

export default function ImportProfessors({ onClose, onSuccess }: ImportProfessorsProps) {
  ...
  const downloadTemplate = () => {
    const template = [
      {
        'Email': 'professor@university.dz',
        'Full Name (Latin)': 'John Doe',
        'Full Name (Arabic)': 'جون دو',
        'Academic Rank': 'Professor',
        'Department': 'قسم العلوم الاقتصادية',
        'Professional Email': 'john.doe@university.dz',
        'Personal Email': 'john.personal@gmail.com',
        'Primary Phone': '+213555123456',
        'Secondary Phone': '+213555654321',
        'PhD Specialization': 'Economics',
    ...
    XLSX.utils.book_append_sheet(wb, ws, 'Professors Template');
    XLSX.writeFile(wb, 'professors_import_template.xlsx');
  };
  ...
  const handleImport = async () => {
    ...
    try {
      ...
      const professors = jsonData.map((row: any) => ({
        email: row['Email'],
        fullNameLatin: row['Full Name (Latin)'],
        fullNameArabic: row['Full Name (Arabic)'],
        academicRank: row['Academic Rank'],
        department: row['Department'],
        professionalEmail: row['Professional Email'],
        personalEmail: row['Personal Email'],
        primaryPhone: row['Primary Phone'],
        secondaryPhone: row['Secondary Phone'],
        phdSpecialization: row['PhD Specialization'],
        fieldOfResearch: row['Field of Research']
      }));

      const response = await fetch('/api/admin/professors/import', {
        method: 'POST',
          ...
          <div className="space-y-6">
            ...
            <div className="bg-blue-50 dark:bg-blue-900/20 border border-blue-200 dark:border-blue-800 rounded-lg p-4">
              <p className="text-sm text-blue-900 dark:text-blue-300 mb-3">
                Download the Excel template, fill in professor data, and upload it here.
              </p>
              <button
